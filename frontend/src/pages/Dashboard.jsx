...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([groupsApi.list(), activityApi.dashboard()])
      .then(([gRes, aRes]) => { setGroups(gRes.data); setActivity(aRes.data) })
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Your groups and recent activity</p>
          </div>
        ...
        <section>
          ...
          {groups.length === 0 ? (
            <div className="rounded-lg border border-dashed p-10 text-center">
              ...
              <p className="text-xs text-muted-foreground mb-4">Create a group to start splitting expenses</p>
              <Button size="sm" onClick={() => setShowCreate(true)}>Create your first group</Button>
            </div>
          ) : (
                ...
                {activity.slice(0, 10).map((item, i) => (
                  <div key={item.id}>
                    <div className="flex items-center gap-4 px-5 py-3.5">
                      ...
                      <span className={`text-sm font-medium shrink-0 ${item.type === 'settlement' ? 'text-green-600' : ''}`}>
